...
class StockPickingWave(models.Model):
    ...
    def send_wave_to_pda(self):
        for wave in self: 
            ...
            if all(x.picking_type_id == wave.picking_ids[0].picking_type_id for x in wave.picking_ids):
                raise ValidationError (_("Diferent picking types"))
            if all(x.state in ('assigned', 'partially_available') for x in wave.picking_ids):
                wave.write({'picking_type_id': wave.picking_ids[0].picking_type_id.id,
                'location_id': wave.picking_ids[0].picking_type_id.default_location_src_id.id,
                'location_dest_id': wave.picking_ids[0].picking_type_id.default_location_dest_id.id})
                ...
                raise ValidationError (_("Not all picks are in ok"))

    pack_operation_ids = fields.One2many(
        'stock.pack.operation', string='Related Packing Operations', compute="_compute_ops")
    picking_type_id = fields.Many2one('stock.picking.type', 'Picking type', store=True)
    ...
    pack_operation_exist = fields.Boolean(
        'Has Pack Operations', compute='_compute_ops',
